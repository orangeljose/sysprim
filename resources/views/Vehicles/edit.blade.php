    ...
    <form>
        ...
        <div class="mb-6">
            ...
            @foreach ($vehicleBrands as $brand)
                <option value="{{$brand->id}}"{{$vehicle->vehicleBrand->id == $brand->id ? 'selected' : ''}}>{{$brand->name}}</option>
            @endforeach
            </select>
    
            <label for="model" class="block mb-2 mt-2 text-sm font-medium text-gray-900 dark:text-gray-400">Modelo</label>
            <select id="id_model" name="id_model" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">

            @foreach ($vehicleModels as $model)
                <option value="{{$model->id}}" {{$vehicle->vehicle_model_id == $model->id ? 'selected' : ''}}>{{$model->name}} - {{$model->year}}</option>
            @endforeach
            </select>
        ...
        <button type="submit" id="btnNewVehicle" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Editar</button>
    ...
    $('#id_brand').on('change', function(e){
        ...
        $.get('/api/vehicle_models/list/' + id,function(data) {
            ...
            $.each(data, function(fetch, regenciesObj){
                console.log(data);
                $('#id_model').append('<option value="'+ regenciesObj.id +'">'+ regenciesObj.name + " - " + regenciesObj.year + '</option>');
            });
        });
    });
</script>
